...
// Real capture (2026-08-12), local air fry with a 1h6min timer: R2 has
// seconds=0, minutes=7, [18]=1 (hours), set_min=7, [20]=1 (set_hours).
const LOCAL_1H6M_TIMER_HEX =
    ''

// Real capture (2026-08-12), the instant the timer above was reset to a flat
// 2h: R2 has seconds=0, minutes=0, [18]=2 (hours), set_min=0, [20]=2 (set_hours).
const LOCAL_2H_RESET_HEX =
    ''
...
describe(MODEL_ID, () => {
    ...
    test('timers over 1h are read from the hours byte, not just minutes', () => {
        const { ha, thinq, dev } = makeDevice()

        thinq.emit('data', buf(LOCAL_1H6M_TIMER_HEX))
        let p = ha.devices[DEVICE_ID].properties
        assert.equal(p['remaining-'], 67, '1h7m (66-min setting, caught a second into the countdown) = 67 min')
        assert.equal(p['set_timer-'], 67, 'set_min=7 + set_hours=1 -> 67 min total')

        thinq.emit('data', buf(LOCAL_2H_RESET_HEX))
        p = ha.devices[DEVICE_ID].properties
        assert.equal(p['remaining-'], 120, 'reset to a flat 2h -> 120 min')
        assert.equal(p['set_timer-'], 120)
